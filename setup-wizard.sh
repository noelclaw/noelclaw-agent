...
step_header 4 4 "TELEGRAM  (optional)"
...
cat > "$ENV_FILE" << EOF
# noelclaw environment — updated $(date -u +%Y-%m-%dT%H:%M:%SZ)
PRIVATE_KEY=${KEYPAIR}
...
echo -e "  ${DIM}  as service${NC}    systemctl --user enable --now noelclaw"
echo ""
