// lib/telegram.js — Telegram channel client for noelclaw
...
function start(token, agentCtx) {
  ...
  bot.command('help', ctx => {
    ...
    ctx.reply(
      '*noelclaw commands*\n\n' +
      '/wallet — check ETH \\+ NOELCLAW balances\n' +
  ...
  bot.command('wallet', async ctx => {
    ...
    try {
      ...
      const msg = `*Wallet*\n\`${b.address}\`\n\nETH: \`${b.eth?.toFixed(6) ?? '0.000000'}\`\nNOELCLAW: \`${(b.noelclaw ?? b.circuit ?? 0).toLocaleString()}\``;
