...
class WalletManager {
  ...
  async getCircuitBalance() {
    try {
      ...
      log('warn', 'NOELCLAW balance check failed', { error: err.message });
      return 0;
    }
  }
  ...
  async getBalances() {
    ...
    return { sol: eth, eth, noelclaw: circuit, circuit, address: this.address };
  }
  ...
  async logBalances() {
    ...
    log('info', 'Balances', {
      ...
      noelclaw: b.noelclaw.toLocaleString() + ' NOELCLAW',
    });
    return b;
  }
