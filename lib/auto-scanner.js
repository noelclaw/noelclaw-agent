// lib/auto-scanner.js — Autonomous market scanner + auto-buyer for noelclaw on Base
// Runs every scanIntervalMs (default 5min).
// Scans for dip-reversal candidates, runs GoPlus rug check, auto-buys best.
...
// Tokens that failed with "no Uniswap v3 pool" — skip for the rest of the session
const _noPoolBlacklist = new Set();
...
async function runCycle(api, wallet, swap, cfg, notify) {
  ...
  // Filter by minimum score threshold and no-pool blacklist
  const topScore = scored[0]?._score ?? 0;
  scored = scored.filter(c => c._score >= minScanScore && !_noPoolBlacklist.has(c.mint));
  if (!scored.length) {
    const blacklisted = _noPoolBlacklist.size ? ` (${_noPoolBlacklist.size} token(s) blacklisted — no v3 pool)` : '';
    log('info', `No candidates met minScanScore ${minScanScore} — top was ${topScore}${blacklisted}`);
    return;
  }
  ...
  // Rug check: GoPlus (Base chain) + token-info (non-blocking — proceed on error)
  let rugVerdict = best.verdict ?? best.rugRisk ?? 'unknown';

  // GoPlus check runs first (free, supports Base/EVM)
  ...
  try {
    ...
    const pricePerToken  = Number(result.outAmount) > 0 ? actualSolSpent / Number(result.outAmount) : 0;
    ...
    notify(
      `✅ *${symbol}* bought\n` +
      `${(result.inAmount ?? finalBudget).toFixed(6)} ETH → ${Number(result.outAmount ?? 0).toLocaleString()} tokens\n` +
    ...
    // "missing revert data" on estimateGas = no Uniswap v3 pool for this token
    const isNoPool = err.message.includes('missing revert data') || err.message.includes('estimateGas');
    if (isNoPool) {
      _noPoolBlacklist.add(best.mint);
      log('warn', `No Uniswap v3 pool for ${symbol} — blacklisted for this session`);
      notify(`⚠️ *${symbol}* has no Uniswap v3 pool — skipping in future scans`);
    } else {
