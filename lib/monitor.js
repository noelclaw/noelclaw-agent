...
function start(cfg, agentCtx, telegramBot = null) {
  ...
  log('info', `Position monitor started — checking every ${intervalMs / 1000}s (prices via DexScreener/base)`);
  ...
  const tick = async () => {
    ...
    // Fetch prices via DexScreener (free, supports fast polling)
    // Falls back to api.tokenPrices if DexScreener is unavailable.
    let priceMap = {};
    try {
      priceMap = await _fetchDexscreenerPrices(mints);
      log('info', `Prices fetched (DexScreener) for ${mints.length} position(s)`);
    } catch (dexErr) {
      log('warn', 'DexScreener failed, trying fallback', { error: dexErr.message });
      try {
        const result = await api.tokenPrices(mints);
        priceMap = result.prices ?? {};
        log('info', `Prices fetched (fallback) for ${mints.length} position(s)`);
      } catch (err) {
