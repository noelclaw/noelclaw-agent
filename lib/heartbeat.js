// lib/heartbeat.js — Deterministic heartbeat for noelclaw
//
...
//   - ETH balance below survival warning level
//
...
// Handles scientific notation strings (e.g. "8.549e+22") from legacy positions
function safeBigInt(val) {
  const s = String(val ?? 0);
  if (s.includes('e') || s.includes('E') || s.includes('.')) return BigInt(Math.round(Number(s)));
  return BigInt(s);
}
...
async function buildStatus(api, wallet, cfg) {
  ...
  for (const [mint, pos] of Object.entries(held)) {
    ...
    if (currentPrice != null && pos.solSpent) {
      ...
      const uiAmt    = Number(safeBigInt(pos.tokenAmount ?? 0)) / Math.pow(10, decimals);
  ...
  // ── Wallet balances + low-ETH exception check ──────────────────────────────
  let solBal = null, circuitBal = null;
  try {
    const balances = await wallet.getBalances();
    solBal     = balances.eth ?? balances.sol ?? null;
    circuitBal = balances.noelclaw ?? balances.circuit ?? null;
...
function start(cfg, agentCtx, telegramBot = null) {
  ...
  const internalK  = process.env.NOELCLAW_INTERNAL_KEY ?? process.env.CIRCUIT_INTERNAL_KEY ?? '';
  ...
  const beat = async () => {
    ...
    try {
      ...
      // Without this, a persistent low-ETH or stop-loss warning re-queues the LLM every 5 min.
      if (exceptions.length) {
        const now = Date.now();
